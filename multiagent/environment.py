...
class DynamicMultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, scenario_update, reset_callback=None, reward_callback=None,
                 observation_callback=None, info_callback=None,
                 done_callback=None, shared_viewer=True):

        self.world = world
        self.scenario_update = scenario_update
        self.agents = self.world.policy_agents
        # set required vectorized gym env property
        self.n = len(world.policy_agents)
        # scenario callbacks
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            self._set_action(action_n[i], agent, self.action_space[i])
        # advance world state
        self.scenario_update(self.world)
        self.render()
        self.world.step()
        # record observation for each agent
        for agent in self.agents:
    ...
    def render(self, mode='human'):
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.entities):
                if entity.die:
                    self.render_geoms[e].set_color(*entity.color, alpha=0.0)
                if 'agent' in entity.name:
